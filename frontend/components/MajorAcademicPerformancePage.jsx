...
const semester6CurrentPapers = [
  { code: 'RA3601', subject: 'Robot Dynamics and Control (RDC)', category: 'Core Paper' },
  { code: 'OCS352', subject: 'IoT Concepts and Applications (OE-I)', category: 'Open Elective' },
  { code: 'CRA341', subject: 'Applied Image Processing (PE-V)', category: 'Professional Elective' },
  { code: 'CRA331', subject: 'Robots and Systems in Smart Manufacturing (PE-VI)', category: 'Professional Elective' },
  { code: 'CMR333', subject: 'Computer Aided Inspection and Testing (PE-VII)', category: 'Professional Elective' },
  { code: 'RA3611', subject: 'Robot Kinematics and Dynamics Laboratory (RKD LAB)', category: 'Laboratory' },
  { code: 'RA3612', subject: 'Mini Project', category: 'Project' },
]
...
function MajorAcademicPerformancePage() {
      ...
      <table className="w-full min-w-[560px] text-left text-sm">
        ...
        <tbody>
          {paperCategories.flatMap((category) => {
            ...
            const categoryRow = (
              <tr key={`${title}-${category}`} className="border-t border-slate-800 bg-slate-950/70 text-cyan-200">
                <td className="px-4 py-3 text-center font-semibold" colSpan={4}>{category}</td>
              </tr>
            )

            const dataRows = categoryRows.map((item, index) => (
    ...
    <div className="relative min-h-screen bg-slate-950 text-slate-100">
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-16 sm:px-6">
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 sm:p-8">
          ...
          <h1 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">Major Degree Performance</h1>
          <p className="mt-3 text-sm text-slate-300">B.E. Robotics and Automation</p>
          <p className="mt-2 text-sm text-slate-100"><span className="font-semibold text-cyan-200">Major CGPA:</span> 7.82</p>
          ...
          <div className="mt-6 space-y-3">
            {semesterSections.map((section) => (
              <section key={section.id} className="rounded-xl border border-slate-800 bg-slate-900/55">
                <button
                  type="button"
                  onClick={() => setOpenSemesterId((current) => (current === section.id ? '' : section.id))}
                  className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left"
                  aria-expanded={openSemesterId === section.id}
                >
                  <span className="text-[11px] font-semibold uppercase leading-relaxed tracking-[0.14em] text-cyan-200 sm:text-xs">{section.title}</span>
          ...
          <div className="mt-5 rounded-xl border border-slate-800 bg-slate-900/55 p-4">
            <button
              type="button"
              onClick={() => setOpenCurrentSectionId((current) => (current === 'major-sem6' ? '' : 'major-sem6'))}
              className="flex w-full items-center justify-between gap-3 text-left"
              aria-expanded={openCurrentSectionId === 'major-sem6'}
            >
              <span className="text-[11px] font-semibold uppercase leading-relaxed tracking-[0.14em] text-cyan-200 sm:text-xs">Current Semester (VI) - Major Timetable Papers and Mini Project</span>
...
export default MajorAcademicPerformancePage
